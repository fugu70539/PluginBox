...
const AppleSwitch = ({ isOn, onToggle }: { isOn: boolean; onToggle: () => void }) => (
  <button onClick={onToggle} className={`relative w-[51px] h-[31px] rounded-full transition-colors duration-300 flex items-center px-[2px] ${isOn ? "bg-[#34C759]" : "bg-[#39393d]"}`}>
    <motion.div animate={{ x: isOn ? 20 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} className="w-[27px] h-[27px] bg-white rounded-full shadow-lg flex items-center justify-center">
...
const SettingRow = ({ icon, title, value, onClick, hasArrow = true, children }: any) => (
  <div onClick={onClick} className="w-full h-[52px] flex items-center justify-between px-4 active:bg-white/5 transition-colors group">
...
export default function Settings({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    if (tg?.BackButton) {
      tg.BackButton.show();
      tg.BackButton.onClick(view === "list" ? onBack : () => setView("list"));
  ...
  return (
    <div className="w-full min-h-screen bg-[#0a0a0a] pt-6 px-6 font-display">
      <div className="space-y-8">
        {/* Блок: Аккаунт */}
        <section>
          <h3 className="text-[13px] font-bold text-white/20 uppercase tracking-widest ml-4 mb-2">Аккаунт</h3>
        ...
        {/* Блок: Оформление */}
        <section>
          <h3 className="text-[13px] font-bold text-white/20 uppercase tracking-widest ml-4 mb-2">Оформление</h3>
          <div className="mt-glass rounded-[24px] overflow-hidden divide-y divide-white/5">
    ...
    <div className="fixed inset-0 bg-black z-[200] pt-12 px-7 flex flex-col items-center">
      <div className="w-full max-w-sm flex flex-col gap-6">
        <div className="relative w-full h-14 bg-white/5 rounded-2xl border border-white/10 flex items-center px-5 gap-3">
          <input 
            value={val} onChange={(e) => setVal(e.target.value.slice(0, 10))}
          ...
          <select onChange={(e) => setVal(e.target.value)} className="bg-transparent text-white/40 text-[14px] font-bold outline-none">
